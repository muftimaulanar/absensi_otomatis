...
def selesai3():
    intructions.config(text="Selamat datang, selamat belajar!")
def rekamDataWajah():
    wajah = 'dbwajah'
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            cv2.imwrite(wajah + '/' + namaFile, frame)
            ambilData += 1
        ...
        cv2.imshow('Inisialisasi Wajah', frame)
...
def absensiWajah():
    ...
    faceDetector = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')
    faceRecognizer = cv2.face.LBPHFaceRecognizer_create()
    faceRecognizer.read(latih + '/training.xml')
    ...
    while True:
        ...
        cv2.imshow('Absen dengan Face Detection', frame)
...
# judul
judul = tk.Label(root, text="Absensi dengan Face Detection", font=("Roboto",34),bg="#242526", fg="white")
canvas.create_window(350, 80, window=judul)
#credit
made = tk.Label(root, text="Universitas Nasional", font=("Times New Roman",13), bg="black",fg="white")
...
canvas.create_window(100,170, window=label1)
# for entry data nim
...
canvas.create_window(52, 210, window=label2)
...
canvas.create_window(59, 250, window=label3)

global intructions

# tombol untuk rekam data wajah
intructions = tk.Label(root, text="Selamat datang Mahasiswa/i!", font=("Roboto",15),fg="white",bg="black")
canvas.create_window(370, 300, window=intructions)
Rekam_text = tk.StringVar()
Rekam_btn = tk.Button(root, textvariable=Rekam_text, font="Roboto", bg="black", fg="white", height=1, width=15,command=rekamDataWajah)
Rekam_text.set("Inisialisasi wajah")
Rekam_btn.grid(column=0, row=7)

# tombol untuk training wajah
Rekam_text1 = tk.StringVar()
Rekam_btn1 = tk.Button(root, textvariable=Rekam_text1, font="Roboto", bg="black", fg="white", height=1, width=15,command=trainingWajah)
Rekam_text1.set("Training")
Rekam_btn1.grid(column=1, row=7)

# tombol absensi dengan wajah
Rekam_text2 = tk.StringVar()
Rekam_btn2 = tk.Button(root, textvariable=Rekam_text2, font="Roboto", bg="black", fg="white", height=1, width=20, command=absensiWajah)
